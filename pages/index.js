...
var loops = [ 
	{ url: '0-ny-1.ogg' },
	{ url: '8bit_nomad-1.ogg' },
	{ url: 'aybabtu-1.ogg' },
	{ url: 'binarpilot1-1.ogg' },
	{ url: 'binarpilot2-1.ogg' },
	//{ url: 'bionic-1.ogg' }, // есть но чёт совсем плохо для лупа
	{ url: 'holdinon-1.ogg' },
	{ url: 'panorama-1.ogg' },
	{ url: 'your_world-1.ogg' },
	{ url: 'tv_on_the_radio-1.ogg' },
...
audio.stop = function() {
	...
	//audio.source_loop[audio.compatibility.stop](0);
	if (audio.source_loop[audio.compatibility.stop]) {
		audio.source_loop[audio.compatibility.stop](0);
	} else {
		return;
	}
...
var bars = {
	...
	smoothing: 0.2,
	//fft_size: 2048,
	fft_size: nearestPowerOf2(document.querySelector("body").clientWidth*2),
	treshold: 0.65,
...
window.addEventListener('resize', function(event){
	...
	bars.width = bodywidth;
	bars.fft_size = nearestPowerOf2(bodywidth*2);
});
var mobile = false;
...
if (mobile) {
	document.querySelector("#clicker-wrap").setAttribute("style", "display: flex;")
}
...
VisualizerSample.prototype.draw = function() {
	...
	for (var i = 0; i < this.analyser.frequencyBinCount; i++) {
		...
		drawContext.fillStyle = 'rgb(120, 0, 0)';
		//drawContext.fillRect(i*4, y, 2, height);
		drawContext.fillRect(i*1.25, y, 1, height);
	}
	*/
	...
	drawContext.fillRect(0, 0, bars.width, bars.height);
	var barWidth = (bars.width / bufferLength) * 2.5;
	var barHeight;
	var x = 0;
	var y;
	var draw_cell = document.querySelectorAll('.a-cell');
	...
	for(var i = 0; i < bufferLength; i++) {
		...
		var percent = value / 200;
		var height = bars.height * percent;
		y = Math.ceil(bars.height - height);
		if(percent > (bars.treshold - normalizer) && !(i % bars.trimmingConstant)) {
			if (!mobile) {
				$(draw_cell[ i + Math.floor((y/1)*bars.trimmingConstant) ]).flash();
				$(draw_cell[ i + Math.floor((y/2)*bars.trimmingConstant) ]).flash();
				$(draw_cell[ i + Math.floor((y/3)*bars.trimmingConstant) ]).flash();
			}
			$(draw_cell[ i + Math.floor((y/4)*bars.trimmingConstant) ]).flash();
		}
		if (!blue) {
			drawContext.fillStyle = 'rgb(' + (barHeight+50) + ',50,50)';
		} else {
			drawContext.fillStyle = 'rgb(0,170, '+ (barHeight+100) +')';
		}
		drawContext.fillRect(x,(bars.height-barHeight),barWidth,barHeight);
		x += barWidth + 1;
	}

	/*
	document.querySelectorAll('.a-cell').forEach(function(e) {
		e.setAttribute("style", "transform: scale(1."+ y +")");
	});
	*/
	var sizey = Math.ceil(bars.height - bars.height * this.times[0] / 200);
	//document.querySelector("#nullgrid").setAttribute("style", "transform: scale(1.0"+ Math.ceil(bars.height - bars.height * this.times[0] / 200) +")");
	document.querySelector("#nullgrid").setAttribute("style", "transform: perspective(100px) scaleZ(-1.2) translateZ("+sizey/20+"px); filter: contrast("+ (1+(sizey/700))+ ");");
	document.querySelector("#shadow").setAttribute("style", "transform: scale("+(0.3+(sizey/200))+");");
	/*
	document.querySelectorAll('.a-cell').forEach(function(e) {
		e.setAttribute("style", "transform: scale(1."+ y +")");
	});
