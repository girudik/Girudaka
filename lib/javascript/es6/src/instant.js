...
var _messages = {
  en: {
    ...
    settings_miniThumb: "Mini Thumbnails",
    settings_hideCompletely: 'Hide items completely',
    settings_hideDeleted: "Hide deleted items",
    settings_hideAllShow: "Uncollapse all deleted items",
    settings_expandImgFull: "Expand images to full size",
    settings_constrainWidth: "Constrain content width",
    settings_lowVolume: "Low volume for media",
  ...
  ru: {
    ...
    settings_miniThumb: "Мини миниатюры",
    settings_hideCompletely: 'Скрывать треды и посты полностью',
    settings_hideDeleted: "Скрыть удалённые треды и посты",
    settings_hideAllShow: "Развернуть все удалённые треды и посты",
    settings_expandImgFull: "Разворачивать картинки до исходного размера",
    settings_constrainWidth: "Ограничивать ширину контента",
    settings_lowVolume: "Низкая громкость для медиа",
...
const Ajax = {
      ...
      if (res.error) {
        ...
        if (res.thread_replyto != 0) {
          newposts.get({
            ...
            onSuccess: () => $(`#delform_instant div[id^=thread${res.thread_replyto}] .fresh-replies`).remove()
          })
        }
        else {
          if (noko) {
  ...
  reportPost: function(fd, callback=null) {
    ...
    xr.open('POST', document.forms.delform_instant.action)
    xr.onload = e => {
      if (xr.status !== 200) {
...
var PostPreviews = {
  ...
  _mouseover: function(e) {
    ...
    if(e.clientY < ($(window).height() / 1.5)) {
      ...
      //console.log("transform 1 1 2");
      transformOrigin = "top "
    } else {
        //console.log("transform 1 2 1");
      $preview.css({bottom:$(window).height()-e.pageY+5});
      //console.log("transform 1 2 2");
      transformOrigin = "bottom "
    }
    //console.log("transform 2");
    if(e.clientX < ($(window).width() / 1.5)) {
        //console.log("transform 2 1 1");
      $preview.css({left:e.pageX+15});
      //console.log("transform 2 1 2");
      transformOrigin += "left"
    } else {
        //console.log("transform 2 2 1");
      $preview.css({right:$(window).width()-e.pageX+15});
      //console.log("transform 2 2 2");
    ...
    //console.log("HTMLoader.getPost start");
    ...
    //console.log("HTMLoader.getPost end");
    //console.log("_mouseover end");
    e.preventDefault();
  },
...
var Settings = {
  ...
  miniThumb: function(changed) {
    var enabled;
    if (changed != null && ismobile) { // force for mobile
        enabled = Settings._checkbox(changed, 'miniThumb', true);
    } else {
        enabled = Settings._checkbox(changed, 'miniThumb', false);
    }
    if (enabled) {
      injector.inject('miniThumb', 'img.thumb:not([alt^="full"]), img.embed-thumbnail { max-width: 200px; max-height: 200px; object-fit: scale-down; object-position: top; width: inherit; height: inherit; }');
    } else {
      injector.remove('miniThumb');
    }
    return enabled;
  },
  ...
  hideDeleted: function(changed) {
    var enabled = Settings._checkbox(changed, 'hideDeleted', true);
    if (changed != null) {
        if (enabled) {
          injector.inject('hideDeleted', 'details.deleted { display: none; visibility: hidden; pointer-events:none; }');
        } else {
          injector.remove('hideDeleted');
        }
    }
    return enabled;
  },
  hideAllShow: function(changed) {
    var enabled = Settings._checkbox(changed, 'hideAllShow', false);
    if (changed != null) {
        if (enabled) {
          document.body.querySelectorAll('details.deleted').forEach((e) => { e.open = true; });
        } else {
          document.body.querySelectorAll('details.deleted').forEach((e) => { e.open = false; });
        }
    }
    return enabled;
  },
  ...
  lowVolume: function(changed) {
    return Settings._checkbox(changed, 'lowVolume', false);
  },
}
...
var rswap = {
  i: true,
  swap: function() {
    if(this.i) $('#delform_instant').before($('#rswapper')).after($('.postarea'));
    else $('#delform_instant').before($('.postarea')).after($('#rswapper'));
    this.i = !this.i;
  }
}
...
function readyset() {
  ...
  .on('click', '.uib-tx', function() {
    ...
    head.js('https://latex.codecogs.com/editor3.js', function() {
  ...
  .on('click', '.menu-delete', function() {
    ...
    fd.append('postpassword', $pass.length
      ? $pass.find('input').val()
      : $(`#delform_instant input[name="postpassword"]`).val())
  ...
  $('#delform_instant').on('submit', function(e) {
    e.preventDefault()
  ...
  $('.userdelete input[type=submit]').click(function(e) {
    ...
    if (this.name == 'moddelete') {
      fd.append('moddelete', true)
      Ajax.deleteItems(fd)
    }
  ...
  if (!is_overboard)
    $('#delform_instant').after('<div id="rswapper">[<a onclick="javascript:rswap.swap();return false;" href="#">'+(ispage ? _l.NewThread : _l.reply)+'</a>]<hr /></div>');
  ...
  if (localStorage) {
    ...
    $("#js_settings").append("<span style=\"color:#F00\">"+_l.noLocalStorage+"</span><br />Твой браузер — говно. Скачай <a href=\"/web/20110329072959/https://google.com/chrome\" target=\"_blank\">Chome</a>, например.");
  }
  ...
  $('#delform_instant').on('click', '.postertrip', function(ev) {
    ev.preventDefault()
    ev.stopPropagation()
      ...
      if ($this.length) {
        NTP.getServerTime().then(srvTime => {
          ...
          if (ms_left >= 0) {
            ...
            $.get(document.forms.delform_instant.action) // call board.php to force delete posts
...
const updater = {
  ...
  notifyAboutNewThreads: function(data) {
    ...
    if (! document.hasFocus()) {
      ...
      $('#delform_instant').before('<a class="xlink" onclick="updater.showNewThreads();return false" id="wild_thread_appeared"><span class="salient">'+_l.newThreadsAvailable+'</span><hr /></a>')
    }
  },
  ...
  showNewThreads: function(callback) {
    ...
    this.newThreads.forEach(thr => {
      HTMLoader.getThread(thr.board, thr.thread, null, (err, posts) => {
        ...
        $wta.remove()
        document.querySelector('#delform_instant').insertAdjacentHTML('afterBegin', `
            ${posts}
        ...
        if (is_overboard && thr.board_desc) {
          document.querySelector('#delform_instant .posthead').insertAdjacentHTML('afterbegin', 
...
function deleteItems(items, bySelf=true, markOnly=false) {
  ...
  items.forEach(item => {
    ...
    if (item.action == 'delete_post' || item.itemtype == 'post') {
      ...
      let $thread = $(`#delform_instant [id^=thread${item.id}]`)
      , $reply = $(`#delform_instant [id^=reply${item.id}]`)
      if ($thread.length) {
        if (markOnly || !ispage) {
        ...
        if (markOnly)
          ...
          let $reply = $(`#delform_instant [id^=reply${item.id}]`)
          if ($reply.length) {
...
var replyMap = {
  showReplies: function(root=document) {
    root.querySelectorAll('.postnode:not(.deleted_1)').forEach(post => {
...
var LatexIT = {
  ...
  pre : function(eqn) {
    ...
    if ( !txt.match(/<img.*?>/i) && !txt.match(/<object.*?>/i))
    {
      ...
      txt = " <img src=\"https://latex.codecogs.com/"+this.mode+".latex?"+ extxt +"\" title=\""+this.dcls+"\" alt=\""+atxt+"\" ondblclick=\""+this.odc+"\" border=\"0\" class=\"latex\" /> ";
    }
    return txt;
  },
...
function unwrapEmbed($fig) {
  ...
  if ($iw.data('site') == "Coub")
    $iw.append(`<iframe src="https://coub.com/embed/${code}?muted=false&autostart=true&originalSize=false&hideTopBar=false&noSiteButtons=false&startWithHD=false" ${iframeOptions}></iframe>`)
...
var embedLinks = {
  sites: [
    {id: 'youtube', rx: /(?:youtu(?:\.be|be\.com)\/(?:.*v(?:\/|=)|(?:.*\/)?)([\w'-]+)|^([a-z-A-Z0-9_-]{11})$)/i },
...
var catalog = {
  ...
  load: function() {
    ...
    this.model = null;
    // get contents
    $.getJSON('/' + this_board_dir + '/catalog.json?v='+(new Date().getTime()))
  ...
  getEmbedThumb: function($el) {
    ...
    if(site == 'cob')
      $.get(ku_boardspath+'/corpsy.php?code='+id, (function(res) {
        $el.replaceWith('<img loading="lazy" width="50" height="50" style="object-fit: cover;" src="'+res.thumbnail_url+'">');
        this.rendered[this.conf.layout][threadID] = $thread[0].outerHTML;
      }).bind(this));
    if(site == 'vim')
      $.get('https://vimeo.com/api/v2/video/'+id+'.json', (function(res) {
        $el.replaceWith('<img loading="lazy" width="50" height="50" style="object-fit: cover;" src="'+res[0].thumbnail_medium+'">');
        this.rendered[this.conf.layout][threadID] = $thread[0].outerHTML
      }).bind(this));
  },
    ...
    if(!thread.processed) {
      ...
      if(!embed || embed === 'removed') {
        ...
        if (
          ...
          thread.smallThumb =
          '<a href="'+thread.url+'" class="smallThumb">\
            <img loading="lazy" width="50" height="50" style="object-fit: cover;" src="/' + this_board_dir + '/thumb/'+thumbURL+'c.'+ftype+'"'+vartype+'>'
          +'</a>';
          thread.bigThumb =
          '<img loading="lazy" width="200" height="200" style="object-fit: cover;" src="/' + this_board_dir + '/thumb/'+thumbURL+'s.'+ftype+'"'+vartype+'>';
        }
        /*if(_.includes(this.fileTypes.image, thread.file_type)) {
          thread.fileTypeClass = 'image';
          thread.smallThumb =
          '<a href="'+thread.url+'" class="smallThumb">\
            <img loading="lazy" width="50" height="50" style="object-fit: cover;" src="thumb/'+thread.file+'c.'+(thread.file_type === 'webm' ? 'jpg' : thread.file_type)+'">'
          +'</a>';
          thread.bigThumb =
          '<img loading="lazy" width="50" height="50" style="object-fit: cover;" src="thumb/'+thread.file+'s.'+(thread.file_type === 'webm' ? 'jpg' : thread.file_type)+'">';
        }*/
        //  small thumbnail for embeds and generic files
          ...
          thread.smallThumb =
          '<a href="'+thread.url+'" class="smallThumb">\
            <img loading="lazy" width="50" height="50" style="object-fit: cover;" src="'+smallSrc+'">' +
        ...
        if(_.includes(this.fileTypes.embed, thread.file_type)) {
          ...
          ? '<img loading="lazy" width="50" height="50" style="object-fit: cover;" src="https://i.ytimg.com/vi/'+thread.file+'/mqdefault.jpg">'
...
var unreadCounter = {
  init: function(built) {
    if (!built) {
      ...
      built = Math.floor(Date.now() / 1000);
    }
    //this.lastvisits = localStorage['lastvisits'] ? (JSON.parse(localStorage['lastvisits']) || { }) : { }
    this.lastvisits = localStorage['lastvisits'];
    if (this.lastvisits === undefined) {
      this.lastvisits = {};
      let boardids = [].map.call(document.querySelectorAll('#boardlist_header .menu-item'), e => e.dataset.id);
      for (var i = 0; i < boardids.length; i++) {
        this.lastvisits[boardids[i]] = built;
      }
      localStorage.setItem('lastvisits', JSON.stringify(this.lastvisits))
    } else {
        this.lastvisits = JSON.parse(localStorage['lastvisits']);
    }
...
var HTMLoader = {
  loadThread: function(boardID, threadID, done, postID) {
    ...
    .then(data => {
      let posts_match = data.match(/<div\s*?class\s*?=\s*?"[^"]*?i0svcel[^"]*?"\s*?>!i0-pd:[0-9]+\s*?<\/div\s*?>[\s\S]+?<div\s*?class\s*?=\s*?"[^"]*?i0svcel[^"]*?"\s*?>!i0-pd-end\s*?<\/div\s*?>/gi);
      let posts;
      if (posts_match) {
        posts = posts_match.map(match => {
          ...
          done("Тред или пост перемещён или не существует");
          return;
      }
      if (threadID === '?') {
