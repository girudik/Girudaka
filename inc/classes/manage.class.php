  ...
  function CheckToken($posttoken) {
	...
	*/
	...
	function movethread() {
		...
		$tpl_page .= '<h2>'. _gettext('Move thread(s)') . '</h2><br />';
		...
		if (isset($_GET['id']) && isset($_GET['board_from']) && isset($_GET['board_to'])) {
			...
			if ($threads_from[count($threads_from)-1] == "") {
				array_pop($threads_from);
			}
			
			
			foreach($threads_from as $thread_from) {
				if (!preg_match('/^[0-9]+$/', $thread_from))
				...
				foreach($posts as $post) {
					...
					foreach($post['embeds'] as $embed) {
						if ($embed['file'] != 'removed') {
							...
							foreach($files as $f) {
								@copy(KU_BOARDSDIR.$board_from.$f, KU_BOARDSDIR.$board_to.$f);
		...
		<label for="id">'. _gettext('IDS by comma') . ':</label>
		<input type="text" name="id" style="display: block; width: 300px;" />
		<br />
		...
		'<br />
		<input type="submit" value="'. _gettext('Move thread(s)') . '" />';
	}
	...
	*/
			...
			foreach ($resultsreport as $linereport) {
				...
				foreach ($results as $line) {
					if ($line['IS_DELETED'] == 0) {
						...
						$tpl_page .= '</td><td><a href="/manage_page.php?action=reports&clear='. $linereport['id'] . '">OK</a>&nbsp;or&nbsp;&#91;<a href="/manage_page.php?action=delposts&boarddir='. $linereport['board'] . '&del'. $post_threadorpost . 'id='. $line['id'] . '" title="Delete" onclick="return confirm(\'Are you sure you want to delete this thread/post?\');">Del</a>&nbsp;<a href="'. KU_CGIPATH . '/manage_page.php?action=delposts&boarddir='. $linereport['board'] . '&del'. $post_threadorpost . 'id='. $line['id'] . '&postid='. $line['id'] . '" title="Delete &amp; Ban" onclick="return confirm(\'Are you sure you want to delete and ban this poster?\');">&amp;</a>&nbsp;<a href="?action=bans&banboard='. $linereport['board'] . '&banpost='. $line['id'] . '" title="Ban">Ban</a>&#93;</td></tr>';
					} else {
	...
	function recentimages() {
		...
		if ($imagesshown <= $_SESSION['imagesperpage']) {
			...
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `" . KU_DBPREFIX . "boards`.`name` AS `boardname`, `" . KU_DBPREFIX . "posts`.`boardid` AS boardid, `" . KU_DBPREFIX . "posts`.`id` AS id, `" . KU_DBPREFIX . "posts`.`parentid` AS parentid, `" . KU_DBPREFIX . "posts`.`file` AS file, `" . KU_DBPREFIX . "posts`.`file_type` AS file_type, `" . KU_DBPREFIX . "posts`.`thumb_w` AS thumb_w, `" . KU_DBPREFIX . "posts`.`thumb_h` AS thumb_h FROM `" . KU_DBPREFIX . "posts`, `" . KU_DBPREFIX ."boards` WHERE (`file_type` = 'jpg' OR `file_type` = 'gif' OR `file_type` = 'png') AND `reviewed` = 0 AND `" . KU_DBPREFIX . "boards`.`id` = `" . KU_DBPREFIX . "posts`.`boardid` ORDER BY `timestamp` DESC LIMIT " . intval($_SESSION['imagesperpage']));
			if (count($results) > 0) {
